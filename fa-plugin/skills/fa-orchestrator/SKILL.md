...
2. **Generate the PDF** by running:
   ```bash
   python3 scripts/generate_fa_pdf.py \
     "company-analysis/{company-slug}/fa-phase1-{YYYY-MM-DD}.md" \
     "company-analysis/{company-slug}/fa-phase1-{YYYY-MM-DD}.pdf"
   ```
   This script scrapes management headshots, product images, and company logo from the company website, then renders a 6-page investor-grade PDF using Chrome headless.
...
- If Chrome is not at `/Applications/Google Chrome.app/...`: update the `CHROME` constant in `scripts/generate_fa_pdf.py`
- If PDF generation fails: save only the `.md` file, inform the user. The script exits with a non-zero code on failure.

## References

- **`references/stock-universe.md`** — All 26 portfolio tickers with display names and sectors
